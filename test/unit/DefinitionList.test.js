...
describe('DefinitionList', () => {
  ...
  it('should display a definition list of detail elements', () => {
    ...
    expect(component.find('tr')).to.have.length(4);
    expect(component.find('td')).to.have.length(4);
    const terms = component.find('th');
    expect(terms.at(0).text()).to.equal(detailDefinitionLabels.agents);
    expect(terms.at(1).text()).to.equal(detailDefinitionLabels.subjects);
    expect(terms.at(2).text()).to.equal(detailDefinitionLabels.date_created);
  ...
  it('should have a table of Items', () => {
    ...
    expect(component.find('tbody tr')).to.have.length(4);
  ...
  it('should have a list of Subjects', () => {
    ...
    expect(subjects.find('Link')).to.have.length(4);
  });
